/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PermissionGroupNameWhereInput } from "./PermissionGroupNameWhereInput";
import { Type } from "class-transformer";
import { PermissionGroupNameOrderByInput } from "./PermissionGroupNameOrderByInput";

@ArgsType()
class PermissionGroupNameFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PermissionGroupNameWhereInput,
  })
  @Field(() => PermissionGroupNameWhereInput, { nullable: true })
  @Type(() => PermissionGroupNameWhereInput)
  where?: PermissionGroupNameWhereInput;

  @ApiProperty({
    required: false,
    type: [PermissionGroupNameOrderByInput],
  })
  @Field(() => [PermissionGroupNameOrderByInput], { nullable: true })
  @Type(() => PermissionGroupNameOrderByInput)
  orderBy?: Array<PermissionGroupNameOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PermissionGroupNameFindManyArgs };
