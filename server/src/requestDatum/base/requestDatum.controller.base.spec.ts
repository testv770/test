...
describe("RequestDatum", () => {
  ...
  test("GET /requestData/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/requestData"}/${nonExistingId}`)
  ...
  test("GET /requestData/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/requestData"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
